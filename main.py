...
# Convert priority values to numeric for the classifier
priority_mapping = {'Low': 0, 'Medium': 1, 'High': 2}
tasks['priority'] = tasks['priority'].map(priority_mapping)

# Check if there are tasks in the CSV file and train the model
if not tasks.empty:
...
def add_task(description, priority):
    global tasks
    new_task = pd.DataFrame({'description': [description], 'priority': [priority_mapping[priority]]})
...
def recommend_task():
    if not tasks.empty:
        ...
        high_priority_tasks = tasks[tasks['priority'] == 2]
